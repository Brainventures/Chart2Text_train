...
# checkpoint_path = "/workspace/Qwen/ex_models/qwen25-3b-ALL_vari_query"
checkpoint_path = "/workspace/Qwen/ex_models/qwen25-3b-ALL_no_vari_query"

quant_config = BitsAndBytesConfig(
    load_in_4bit=True,
    bnb_4bit_quant_type="nf4",     # 또는 "fp4", "fp4-dq"
...
nia_path = '/workspace/data_moder/GPT_ALL.json' # GPT_ALL <- query가 바뀐 GPT
des_path = '/workspace/data_moder/Test_Nia_Chart2ALL_description.json'
sum_path = '/workspace/data_moder/Test_Nia_Chart2ALL_summary.json'
table_path = '/workspace/data_moder/Test_Nia_Chart2ALL_table.json'

data_lst = {'total_path' : nia_path , 'description_path' : des_path, \
            'summary_path' : sum_path , 'table_path' : table_path}
...
for name , path in data_lst.items():
    ...
    eval_num = int(0.0005 * len(dataset["train"]))
    ...
    def evaluate_batch(batch):
        ...
        for image_dir, query, label in zip(batch['image'], batch['query'], batch['label']):
            references.append(label)  # 참조 데이터 저장
            # 메시지 포맷
